...
class PQ4Codec(BaseCodec):
  ...
  def __init__(
    ...
    super().__init__()
    assert 1 <= n_clusters <= 16
    assert n_subvectors % 2 == 0
  ...
  def encode(self, x):
    ...
    labels = labels.reshape(2, self.n_subvectors // 2, n_data)
    labels[0] = labels[0] * 16 + labels[1]
    return labels[0].byte().contiguous()
  ...
  def decode(self, code):
    ...
    """
    assert self._is_trained == True, "codec is not trained"
    assert code.shape[0] == self.n_subvectors // 2
    n_data = code.shape[1]
    first_half = torch.floor_divide(code, 16)
    second_half = code % 16
    code = torch.stack([first_half, second_half], dim=0).reshape(self.n_subvectors, n_data)
